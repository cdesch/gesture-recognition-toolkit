...
bool TimeSeriesGraph::init(unsigned int numPoints,unsigned int numDimensions){
    ...
    dataBuffer.resize(numPoints, vector< double >(numDimensions,0));
    
    //Fill the buffer with empty values
    for(unsigned int i=0; i<numPoints; i++)
        dataBuffer.push_back(vector< double >(numDimensions,0));
...
bool TimeSeriesGraph::reset(){
    ...
    if( !rangesLocked ){
        ...
        //Clear the buffer
        dataBuffer.setAllValues(vector< double >(numDimensions,0));
    }
    
    return true;
}

bool TimeSeriesGraph::setRanges(vector<double> minRanges,vector<double> maxRanges,bool rangesLocked){
...
bool TimeSeriesGraph::update(vector< double > data){
